...
Efron, An Introduction to the Bootstrap. Chapman & Hall 1993
    ...
    if _iter:
        if multiLevel:
            bootindexes = bootstrap_indexes_MultiLevel(tdata[0], n_samples)
        else:
...
def bootstrap_indexes_MultiLevel(data, n_samples=10000):
    """
    Given data points data, where axis 0 is considered to delineate points, return
    an generator for sets of bootstrap indexes. This can be used as a list
    of bootstrap indexes (with list(bootstrap_indexes(data))) as well.
    ...
    for key in count_dict.keys():
        key_index_dict[key] = np.where(data == key)[0]
    
    for _ in xrange(n_samples):
        generated_index_list = list()
        for key in key_index_dict.keys():
            generated_index_list.append(np.random.choice(key_index_dict[key],(key_index_dict[key].shape[0],)))
        
        outputIndex = np.concatenate(generated_index_list,axis=None)
